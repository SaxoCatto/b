...
def run_one(verbose: bool,
    ...
    try:
        # print("Checkpoint 3 in scripts_lib")
        ...
        ps = subprocess.run(cmd,
                            ...
                            env=env)
        if ps.returncode:
            print(f"\n\nThere is some error when run a subprocess with command {cmd_str}\n\n")
            return 0
        else:
        ...
        if needs_closing:
            # print("Checkpoint 3.2 in scripts_lib with needs_closing = 1")
